...
impl<E: JubjubEngine> Seed<E> {
    ...
    pub fn deterministic_seed(pk: &PrivateKey<E>, msg: &[u8]) -> Self {
        ...
        key.copy_from_slice(mac.clone().finalize().into_bytes().as_mut_slice()); // key = HMAC(key, concatenated)

        mac = HmacSha256::new_from_slice(&key).expect("HMAC can take key of any size");
        mac.update(&v);
        v.copy_from_slice(mac.clone().finalize().into_bytes().as_mut_slice()); // v = HMAC(key, v)
        ...
        loop {
            ...
            mac = HmacSha256::new_from_slice(&key).expect("HMAC can take key of any size");
            mac.update(&v);
            k_slice.copy_from_slice(mac.clone().finalize().into_bytes().as_mut_slice()); // k = HMAC(key, v)
